    ...
    for (let i = 0; i < templateData.length; i++) {
      ...
      if (isArchived !== true) {
        templates.push({
          ...
          createdBy: row[2],
          dateCreated: row[3],
          criteria: criteriaMap[templateID] || [] // Attach the list of criteria
        });
      }
    }
...
function webSaveCoachingTemplate(templateObject) {
  try {
    ...
    // 1. Save the Template Header
    if (templateID) {
      // It's an existing template, find the row to update
      ...
      // Delete all old criteria for this template
      ...
      // It's a new template
      templateID = `TPL-${new Date().getTime()}`; // Generate new ID
